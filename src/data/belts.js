...
//
// The *counts* deliberately did not move, and that is worth stating because raising them is
// the obvious thing to reach for when a belt looks empty. Two reasons not to. It inflates
// the ore supply, which is an economy change wearing a rendering change's clothes. And rock
// placement draws `orbitSpeed` from the shared world stream (`wrand`), so adding four
// hundred rocks advances that stream four hundred times and every seeded decision made
// afterwards — NPC spawns, factions, ambushes — lands somewhere else. A belt that looks
// better is not worth a world that is silently a different world.
//
// What actually fixed the empty-belt problem is the clutter tier in `world/asteroids.js`,
// which adds no records and touches no stream.
export const BELTS = [
