 ...
 * Station classes.
 *
 * `forge` and `forgeSize` name the Station Forge archetype this class is built from, and how
 * big a layout to grow (1–5). They live here rather than in world/ because this is already
 * the file that decides what a station *is* — putting the mapping next to `cat` and `size`
 * means one place changes when a class is added, instead of a class here and a translation
 * table over in the generator that nobody remembers to update.
 *
 * The forge's vocabulary is not the game's: it knows military/trade/research/mining/
 * habitat/pirate/derelict, and the game knows economic/military/industrial/logistics/
 * civilian/pirate. Two classes can share an archetype — a refinery and a foundry are both
 * `mining` layouts, at different scales — and `relay` is the one class with no good match,
 * so it takes the smallest research layout it can.
 */
export const STATION_TYPES = {
  tradeHub:  {name:'Trade hub',        cat:'economic',   slots:8, size:34,
              forge:'trade',    forgeSize:3,
              base:['reactor','atmosphere','gravity','market','cargo','landingPad']},
  fortress:  {name:'Fortress',         cat:'military',   slots:9, size:32,
              forge:'military', forgeSize:3,
