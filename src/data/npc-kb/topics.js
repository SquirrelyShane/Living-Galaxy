...
import { realise, quantity, described, place, registerOf, chooseFrom,
         npSlot, clause, imperative, proper, complement } from './grammar.js';
...
// an act, and the facts it concerns — which `data/npc-kb/grammar.js` realises into a sentence.
...
export const TOPICS = {
  ...
  checkIn: {
    ...
    say: [
      ...
      ({ a, b, rel, bucket }) => ({
        act: 'ack',
        register: registerOf(a),
        // Tagged, because these are clauses and fragments rather than noun phrases and the
        // realiser now refuses to drop them into a frame that wants an NP. Untagged they
        // defaulted to NP and produced "There is where I said I would be."
        object: known(rel)
          ? clause(chooseFrom(['nothing moving', 'where I said I would be', 'all quiet on my scope'],
                              `${bucket}:checkReply`))
          : proper(b.name),
        verb: 'hold'
      })
  ...
  oreTip: {
    ...
    say: [
      ({ a, b, bucket }) => {
        // The subject is chosen first and handed to the object as something to avoid, so the
        // two cannot land on the same head noun — both pools contain "rock".
        const face = described('face', 'good', { bucket, det: 'def' });
        return {
          act: 'tip',
          register: registerOf(a),
          subject: face,
          verb: 'read',
          object: described('ore', 'good', { bucket, det: 'none', avoid: face }),
  ...
  haulOffer: {
    ...
    say: [
      ...
      ({ a, rel, bucket }) => ({
        ...
        register: registerOf(a),
        // **Second person.** After the a/b swap `a` is the responder, and the responder is
        // asking the *offerer* whether there is a load — "Have you got a hold?" This said
        // `subject: 'I', person: 1`, which asked about itself, and the polar frame's
        // hardcoded "Is " turned it into "Is I having a can?" Fixing the frame's agreement
        // made it grammatical ("Am I having a can?") and left it asking the wrong ship;
        // the person was wrong at the source.
        subject: 'you',
  ...
  askHelp: {
    ...
    say: [
      ({ a, bucket }) => ({
        ...
        verb: 'take',
        object: npSlot(chooseFrom(['fire', 'hits', 'more than I can hold', 'a working over'],
                                  `${bucket}:distress`)),
        where: place(null, { bucket })
      }),
      ({ a, bucket }) => ({
        ...
        register: registerOf(a),
        verb: 'come',
        object: clause(chooseFrom(['I am coming about', 'burning your way now', 'on my way'],
                                  `${bucket}:reply`))
      })
    ],
  ...
  thanks: {
    ...
    say: [
      ({ a, b, bucket }) => ({
        ...
        verb: 'owe',
        // The complement of "owe", not a standalone NP — "you a favour" is a double object
        // and only makes sense after the verb. Tagged as a clause so the existential and
        // verbless frames, which would render it as "There is you a favour", cannot take it.
        object: complement(chooseFrom(['you for that one', 'you a favour', 'you one'],
                                      `${bucket}:thanks`)),
        vocative: b.name
      }),
      ({ a, bucket }) => ({
        act: 'inform',
        register: registerOf(a),
        object: clause(chooseFrom(['buy me something at the next berth',
  ...
  taunt: {
    ...
    say: [
      ({ a, b, bucket }) => ({
        act: 'warn',
        register: registerOf(a),
        object: clause(chooseFrom(['you are a long way from anywhere friendly',
      ...
      ({ a, rel, bucket }) => ({
        ...
        register: registerOf(a),
        object: known(rel)
          ? clause(chooseFrom(['you again', 'still talking'], `${bucket}:tauntBack`))
          : imperative(chooseFrom(['say that closer', 'come and read it to me'],
                                  `${bucket}:tauntBack`))
      })
    ],
  ...
  // ── a system with a thousand ships in it (v1.02.57) ────────────────
  //
  // Six topics added with the shoal. Every one of them is about something that only became
  // true in this build: there is lane traffic to complain about, there are berths on the
  // mining fields to sell at, and there are enough hulls in a system for "busy" and "quiet"
  // to be observations rather than adjectives.
  //
  // They use the existing lexicon deliberately — `lane`, `berth`, `contact`, `trouble`,
  // `work` are already nouns the realiser knows how to inflect and describe. A topic that
  // needed six new words would be a topic that only ever said them.

  laneReport: {
    channel: 'trade', weight: 11, cooldown: 200,
    when: (a, b) => role(a, 'haul') && (role(b, 'haul') || role(b, 'mine') || armed(b)),
    say: [
      ({ a, b, bucket }) => ({
        act: 'inform',
        register: registerOf(a),
        ...
        agr: { person: 1, number: 'sg' },
        verb: 'read',
        object: described('lane', a.hp < a.maxHp ? 'busy' : 'quiet', { bucket, det: 'def' }),
        where: place(null, { bucket }),
        vocative: b.name
      }),
      ({ a, rel, bucket }) => ({
        ...
        register: registerOf(a),
        object: known(rel)
          ? clause(chooseFrom(['same on my leg', 'it was worse yesterday',
                               'I am on the same run behind you'], `${bucket}:laneBack`))
          : null,
        verb: 'hold'
      })
    ],
    filesFrom: { type: 'gave-tip', weight: 0.5 },
    filesTo:   { type: 'got-tip', weight: 0.6 }
  },

  beltBerth: {
    channel: 'trade', weight: 10, cooldown: 260,
    when: (a, b) => role(a, 'mine') && (role(b, 'mine') || role(b, 'haul')),
    say: [
      ({ a, b, bucket }) => ({
        act: 'tip',
        register: registerOf(a),
        subject: described('berth', 'good', { bucket, det: 'def' }),
        verb: 'take',
        object: npSlot('the whole cut without the burn back in'),
        vocative: b.name
      }),
      ({ a, rel, bucket }) => ({
        act: known(rel) ? 'ack' : 'ask',
        register: registerOf(a),
        object: clause(chooseFrom(['they were paying under book last shift',
                                   'that saves me half a day',
                                   'the refinery there is honest enough'],
                                  `${bucket}:berthBack`))
      })
    ],
    filesFrom: { type: 'gave-tip', weight: 0.7 },
    filesTo:   { type: 'got-tip', weight: 0.9 }
  },

  crowded: {
    channel: 'local', weight: 8, cooldown: 220,
    when: (a, b) => same(a, b) && !role(a, 'fort'),
    say: [
      ({ a, b, bucket }) => ({
        act: 'inform',
        register: registerOf(a),
        ...
        agr: { person: 1, number: 'sg' },
        verb: 'mark',
        // `det: 'indef'` rather than bare. "I am marking busy contact" is missing an
        // article; the noun is countable and the realiser will not add one for you.
        object: described('contact', 'busy', { bucket, det: 'indef' }),
        vocative: b.name
      }),
      ({ a, bucket }) => ({
        act: 'ack',
        register: registerOf(a),
        object: clause(chooseFrom(['half of them are freight',
                                   'it thins out past the field',
                                   'the band has not been this loud in weeks'],
                                  `${bucket}:crowdedBack`))
      })
    ],
  ...
  escortAsk: {
    channel: 'trade', weight: 9, cooldown: 280,
    when: (a, b) => role(a, 'haul') && armed(b) && same(a, b),
    say: [
      ({ a, b, bucket }) => ({
        act: 'ask',
        register: registerOf(a),
        ...
        agr: { person: 2, number: 'sg' },
        verb: 'watch',
        object: described('lane', null, { bucket, det: 'def' }),
        vocative: b.name
      }),
      ({ a, rel, bucket }) => ({
        act: known(rel) ? 'ack' : 'inform',
        register: registerOf(a),
        object: clause(chooseFrom(['I am on that heading anyway',
                                   'stay on my wing and keep your transponder honest',
                                   'I can hold as far as the ring'],
                                  `${bucket}:escortBack`))
      })
    ],
    filesFrom: { type: 'asked-help', weight: 0.9 },
    filesTo:   { type: 'was-asked-help', weight: 0.9 }
  },

  shiftEnd: {
    channel: 'local', weight: 7, cooldown: 400,
    // No hull-condition clause. An earlier draft gated this on `!hurt(a)` and
    // `test/deals.mjs` caught it: its reachability sample ships every role at half hull, so
    // the topic was unsatisfiable by any pair that actually spawns. A worker at the end of a
    // shift is not a worker in good repair — the two facts have nothing to do with each
    // other, and tying them made a whole conversation unreachable for no reason.
    when: (a, b) => same(a, b) && (role(a, 'mine') || role(a, 'build')),
    say: [
      ({ a, b, bucket }) => ({
        act: 'inform',
        register: registerOf(a),
        // First person with a concrete object, not a possessive subject on an intransitive.
        // "My fat bay has sat" is what the latter produced: grammatical, and nothing anybody
        // has ever said on a radio.
        subject: 'I',
        agr: { person: 1, number: 'sg' },
        verb: 'need',
        object: npSlot(chooseFrom(['two more faces', 'one more cut', 'a clean hour'],
                                  `${bucket}:shiftNeed`)),
        where: place(null, { bucket }),
        vocative: b.name
      }),
      ({ a, bucket }) => ({
        act: 'ack',
        register: registerOf(a),
        object: clause(chooseFrom(['two more faces and I am done',
                                   'the cutter has been hot all shift',
                                   'I am running the last one dry and going in'],
                                  `${bucket}:shiftBack`))
      })
    ],
    filesFrom: { type: 'spoke-with', weight: 0.3 },
  ...
  wreckSighting: {
    channel: 'local', weight: 6, cooldown: 320,
    when: (a, b) => same(a, b) && (hurt(a) || armed(a)),
    say: [
      ({ a, b, bucket }) => ({
        act: 'warn',
        register: registerOf(a),
        object: clause(chooseFrom(['there is a hull out here that did not make it',
                                   'somebody left debris on this bearing',
                                   'I am reading scrap where there should be a freighter'],
                                  `${bucket}:wreck`)),
        where: place(null, { bucket }),
        vocative: b.name
      }),
      ({ a, rel, bucket }) => ({
        act: known(rel) ? 'ack' : 'warn',
        register: registerOf(a),
        object: clause(chooseFrom(['log it and stay off that bearing',
                                   'whoever did it may still be sitting on it',
                                   'that is the third this month'],
                                  `${bucket}:wreckBack`))
      })
    ],
    filesFrom: { type: 'gave-tip', weight: 0.6 },
    filesTo:   { type: 'got-tip', weight: 0.7 }
  }
};
