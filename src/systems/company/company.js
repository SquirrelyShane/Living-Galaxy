...
function pickHQStation(charter) {
...
export function suggestName(ch) {
...
export function restoreCompany(data) {
  ...
  S.company = Object.assign({
    ...
    fleet: [], upkeepT: 0, projects: []
  }, data);
  // A company saved before v1.01.80 has no roster key at all.
  if (!Array.isArray(S.company.fleet)) S.company.fleet = [];
  // ...and one saved before v1.02.35 has no construction order book.
  if (!Array.isArray(S.company.projects)) S.company.projects = [];
  return true;
}
