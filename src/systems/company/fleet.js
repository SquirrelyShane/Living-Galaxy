...
import { holdMass, holdCap } from '../trade/holds.js';
...
export function canCommission(role) {
  ...
  // `COMPANY.commissionRange` has been declared since v1.00.31 and read by nobody — the docked
  // check above was doing all the work, which made the range a plausible-looking knob that
  // changed nothing. It matters for the case the docked check misses: a hull ordered while
  // drifting away from the berth it is supposed to be laid down at. Closed at v1.02.43.
  const berth = S.docked.position;
  if (berth) {
    const away = S.player.position.distanceTo(berth) / 1000;
    if (away > COMPANY.commissionRange) {
      return { ok: false,
               reason: `Too far from the yard — ${Math.round(away)} km out, ${COMPANY.commissionRange} km allowed.` };
    }
  }
  if (roster().length >= COMPANY.fleetCap) {
    return { ok: false, reason: `Fleet is full at ${COMPANY.fleetCap} hulls. Release one first.` };
...
export function commissionHull(role) {
  ...
  const st = S.docked;
  const ship = spawn('npc', spec.hull, Math.floor(stream('fleet').next() * 89) + 10);
...
const hullDocked = u => !!(u && u.dockedAt);
...
const companyHulls = () => roster().filter(c => ownerOf(c) === OWNER.COMPANY);
