...
// Circular by design, and safe: character.js imports this module for the player's record,
// and this module needs exactly one number back — how far through the current rank they
// are. Duplicating the rank-cost curve here to break the cycle would put the progression
// formula in two files, which is the failure this project has paid for twice already.
// Nothing here calls it during module evaluation, so the live binding is resolved by the
// time it is read.
import { rankProgress } from '../crew/character.js';
 ...
 *
 * ## The part in progress counts (v1.02.39)
 *
 * It used to be `rank / 10` and nothing else, which made this function a liar in a way
 * that only showed up once something read it in anger. Proficiency took **ten values**,
 * so a ladder asking 0.35 and a ladder asking 0.40 were the same gate, 0.72 and 0.88 were
 * the same gate, and half the precision in `LADDER` was decoration.
 *
 * Worse than the arithmetic was what a player saw. Measured on the enforcer path: **32
 * completed contracts before the number moved at all**, then a jump. Thirty-one jobs of a
 * radar that does not twitch, a bar that does not fill, and a locked contract that says
 * "you are at 30%" in exactly the same words it said thirty jobs ago. That reads as a
 * broken game, and it was the first thing the loop this patch closes would have done to
 * anybody who played it.
 *
 * `rankProgress()` — the fraction of the way to the next rank — has existed since 0.6 and
 * was read by the skill sheet alone. Folding it in makes competence genuinely continuous:
 * every completed job moves the number, every screen that reads it moves, and the gap to
 * a gate is a distance rather than a wall.
 */
function liveProficiency() {
  ...
  for (const k of SKILL_KEYS) {
    ...
    const rank = base + spent;
    // Only the earned part carries a fraction. Points *spent* from the character sheet are
    // whole ranks by definition, and a partial rank at the ceiling would read as over 100%.
    const partial = rank >= CHAR.maxRank ? 0 : rankProgress(k);
    out[k] = Math.max(0, Math.min(1, (rank + partial) / 10));
  }
  return out;
}
 ...
 */
/**
 * Does this individual hold a qualification — by either of the two ways of holding one?
 *
 * A certificate is either **awarded** to you as a one-off (`award()`, for the things a
 * story or an inspection hands over) or **carried by the rung you have reached**, which is
 * what `grants` on a ladder rung has meant since v1.02.36.
 *
 * They were two vocabularies until v1.02.39 and only one of them was ever asked. Climbing
 * to Marked Gun granted `bounty-low`, `grantsOf()` reported it, the dossier screen printed
 * it — and `qualifies()` looked only at `d.quals`, so the contract that wanted `bounty-low`
 * stayed locked for a character visibly holding it. Exactly the fault this patch exists to
 * fix, reproduced one layer down: a thing declared, displayed, and read by nobody.
 */
export function holds(d, qual) {
  if (!d) return false;
  return (d.quals || []).includes(qual) || grantsOf(d).includes(qual);
}
...
export function qualifies(d, req) {
  ...
  for (const q of (req.quals || [])) {
    if (!holds(d, q)) missing.push({ kind: 'qual', key: q });
  }
  ...
  // A shortfall is floored and a requirement is ceilinged, never rounded to nearest.
  // Rounding produced "Needs gunnery at 40% — you are at 40%", which is a refusal that
  // reads as a bug: the real values were 0.400 and 0.395, and the honest rendering of a
  // near miss is 39%. Since v1.02.39 competence moves continuously, so near misses are the
  // *normal* case for a locked contract rather than a corner one.
  const floor = v => Math.floor(v * 100);
  const ceil = v => Math.ceil(v * 100);
  const why =
    first.kind === 'skill' ? `Needs ${first.key} at ${ceil(first.need)}% — you are at ${floor(first.have)}%.`
    : first.kind === 'standing' ? `Needs standing ${Math.ceil(first.need)} with ${first.name} — you are at ${Math.floor(first.have)}.`
