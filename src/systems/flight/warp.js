...
import { canWarp, announce } from '../platform/preflight.js';
import { intercept } from './ephemeris.js';
import { warpFlash } from '../../world/particles.js';

const _wf = new THREE.Vector3();
import { wearWarp } from '../combat/wear.js';
 ...
 * Lay a course.
 *
 * @param {object} obj      what to fly to
 * @param {string} name     what to call it out loud
 * @param {number} [standoff] how big a gap to leave on arrival, in units. Omitted means
 *   "as close as is safe" — see `arrivalRadius`. The warp menu is what passes it.
 */
export function setCourse(obj, name, standoff) {
  const w = S.warp;
  const wasFlying = w.state === 'warping' && !!w.dest;
  w.standoff = (typeof standoff === 'number' && isFinite(standoff)) ? standoff : null;
...
function dropOut(msg) {
  ...
  const w = S.warp, p = S.player;
  w.state = 'cooldown';
  warpFlash(S.player.position, forward(S.player.yaw, S.player.pitch, _wf), 0.6);
 ...
 */
/**
 * The descriptive record for a warp destination, whichever shape it is in.
 *
 * Most things you fly to are `THREE.Object3D`s carrying their facts on `userData`. Asteroids
 * are not — `world/asteroids.js` builds them as plain records with `kind`, `name`, `radius`
 * and `position` at the top level, because there are hundreds of them per belt and a mesh
 * each would be unaffordable.
 *
 * Warp only ever read `obj.userData`, so with a rock as the destination it saw an empty
 * object: radius 0, no gravity. Harmless-looking, and it meant the arrival check aimed at the
 * exact geometric centre of the rock rather than its surface. One accessor so the two shapes
 * cannot be handled differently by accident again.
 */
export const destInfo = obj => (obj && (obj.userData || obj)) || {};

/**
 * How far out this jump is meant to stop, in units of gap from the destination's hull.
 *
 * `S.warp.standoff` is what the pilot asked for on the warp menu; null means WARP TO, which
 * is "as close as is safe". Read through a function because three places want it and one of
 * them is the course planner, which runs before the jump is committed.
 */
export const warpStandoff = () =>
  (typeof S.warp.standoff === 'number' && isFinite(S.warp.standoff))
    ? Math.max(WARP.closeArrive, S.warp.standoff)
    : WARP.closeArrive;

/**
 * How close a warp course actually gets.
 *
 * The floor used to be `WARP.arriveRadius` — 240 units, unconditionally, for everything.
 * That is right for a body with a well and badly wrong for a station, which has none: it
 * put every berthing jump 240 km short and left the last leg to a quarter-throttle burn.
 *
 * Three inputs now, largest wins: what the pilot asked for, the destination's own gravity
 * well, and — only for things that *have* a well — the old floor. A berth has neither a
 * well nor a mass shadow, so WARP TO can legitimately put you six kilometres off its hull.
 */
export function arrivalRadius(u, asked) {
  const want = typeof asked === 'number' ? Math.max(WARP.closeArrive, asked) : warpStandoff();
  const well = u && u.gravity ? Math.max(WARP.arriveRadius, wellRadius(u) * WARP.arriveFactor) : 0;
  return Math.max(want, well);
}
...
export function updateWarp(dt) {
  ...
  if (w.state === 'spooling') {
    ...
    if (w.charge >= 100) {
      w.state = 'warping';
      // The core letting go. EM blue, because that is what a warp event is on this palette,
      // and it fires at the moment of transition rather than during the spool — the spool
      // already has a bar, the transition had nothing.
      warpFlash(S.player.position, forward(S.player.yaw, S.player.pitch, _wf), 1);
      status(w.dest ? `Warping to ${w.dest.name}` : 'Warp engaged');
    }
    return;
  }
  ...
  const destU = w.dest && destInfo(w.dest.obj);
  // A rock has no gravity worth a well, so `destEdge` is 0 for one — which is correct, and
  // was already the behaviour by accident. It is deliberate now.
  const destEdge = destU && destU.gravity ? wellRadius(destU) * WARP.arriveFactor : 0;
  for (const b of S.world.bodies) {
    const u = b.userData;
    if (u.kind !== 'planet' && u.kind !== 'star' && u.kind !== 'moon') continue;
