...
import { tryTool, toolManifest } from '../platform/tools.js';
import { hasCompany, companyReport } from '../company/company.js';
...
const state = {
  worker: null, ready: false, loading: false, device: null, model: null,
  // A *list* of listeners, not one.
  //
  // This was a single callback, and the last caller of `initAssistant` won. That was fine
  // while the ARIA panel was the only thing that cared. It stopped being fine in v1.02.55,
  // when the loading screen started reporting model download progress too: `ui/aria.js`
  // registers at boot, `main.js` registers a moment later, and whichever ran second
  // silently muted the other. Two subscribers to one event is not a conflict, and the fix
  // is not to pick a winner.
  listeners: [], pending: new Map(), nextId: 1
};

function emit(s) {
  for (const fn of state.listeners) {
    try { fn(s); } catch (e) { /* a broken listener must not stop the others */ }
  }
}
...
export function initAssistant(onStatus) {
  if (typeof onStatus === 'function' && state.listeners.indexOf(onStatus) < 0) {
    state.listeners.push(onStatus);
    // Late subscribers get the current state rather than waiting for the next transition.
    // The loading screen registers after the model may already be in flight, and a screen
    // that shows nothing until the *next* progress packet looks like a screen that is not
    // wired up.
    if (state.ready) onStatus({ kind: 'ready', device: state.device, model: state.model });
    else if (state.loading) onStatus({ kind: 'loading', pct: 0 });
  }
  return () => {
    const i = state.listeners.indexOf(onStatus);
 ...
 * Pull the weights.
 *
 * @param {string} [key] a key into `npc-avatar/llm/models.js`. The boot sequence passes
 *   `AVATAR.bootModel` — the 135M — and the Lab passes whatever the player picked.
 */
export function loadModel(key) {
  if (state.ready || state.loading) return;
  if (typeof Worker === 'undefined') { emit({ kind: 'nofallback' }); return; }
  state.loading = true;
  state.model = key || AVATAR.bootModel || AVATAR.model;
  emit({ kind: 'loading', pct: 0 });
  try {
    state.worker = new Worker(new URL('./assistant.worker.js', import.meta.url), { type: 'module' });
  } catch (e) {
    state.loading = false;
    emit({ kind: 'error', msg: 'worker failed to start' });
    return;
  }
  state.worker.onmessage = ev => {
    const m = ev.data;
    if (m.t === 'progress') emit({ kind: 'loading', pct: m.pct, file: m.file });
    else if (m.t === 'ready') {
      state.ready = true; state.loading = false; state.device = m.device;
      state.model = m.model || state.model;
      emit({ kind: 'ready', device: m.device, model: state.model, params: m.params });
    } else if (m.t === 'reply') {
      ...
      emit({ kind: 'error', msg: m.msg });
    }
  };
  state.worker.postMessage({ t: 'load', model: state.model });
}
...
export function ask(prompt) {
  ...
  return new Promise(resolve => {
    ...
    state.worker.postMessage({ t: 'ask', id, model: state.model, system: SYSTEM,
                               prompt: `${facts}\n\nPilot: ${prompt}` });
  });
}
