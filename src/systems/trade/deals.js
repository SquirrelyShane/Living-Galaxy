...
import { personaFor, noteEvent } from '../npc/npc-brain.js';
import { relation } from '../npc/npc-comms.js';
...
export function willAccept(a, b, deal) {
  ...
  return deal.pay / worth >= acceptanceBar(a, b);       // 1.0 = paying exactly market
}

/**
 * The rate `b` needs before they will carry for `a`, as a fraction of what the cargo is
 * worth at the destination.
 *
 * Split out of `willAccept` at v1.01.98 so the fee the UI suggests can be quoted against the
 * same number the answer is judged by. It could not before, and the gap was visible: the
 * static quote was `baseBar * suggestMargin` = 0.425, while a hauler with greed above ~0.48
 * needs more than that. The game quoted a fee and then told you nobody would take it.
 */
export function acceptanceBar(a, b) {
  ...
  return DEALS.baseBar + greed * DEALS.barPerGreed
 ...
 */
function defaultOn(deal, reason = 'expired') {
 ...
 *
 * Quoted against the band rather than against a constant. `postPlayerJob()` walks every
 * hauler, so the fee that matters is the one the *most amenable* of them will take — and
 * that is a real number available right now, not an average. The static margin survives as
 * the fallback for an empty band, where there is nobody to quote against.
 */
export function suggestedFee(commodity, kg, dest) {
  const worth = dealValue({ commodity, kg, dest });
  const haulers = ((S.world && S.world.npcs) || [])
    .filter(n => n.userData && n.userData.role === 'haul' && n.userData.hp > 0);

  let bar = DEALS.baseBar * DEALS.suggestMargin;
  if (haulers.length) {
    const best = Math.min(...haulers.map(h => acceptanceBar(PLAYER_U, h.userData)));
    // A hair over the bar rather than exactly on it: `willAccept` is `>=`, but the fee is
    // rounded to credits and the cargo's worth moves with the destination market between
    // the quote and the post.
    bar = Math.max(best * DEALS.quoteMargin, DEALS.baseBar);
  }
  return Math.round(worth * bar);
}
