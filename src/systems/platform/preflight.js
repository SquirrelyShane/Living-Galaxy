...
import { feedOf, feedLoaded } from '../combat/magazine.js';
import { firingSlots, activeLabel } from '../combat/groups.js';
import { WEAPON_MODULES } from '../../data/weapons.js';
import { warpBlocked } from '../industry/habitat.js';
import { status } from '../../core/notify.js';
import { sfx } from './audio.js';
import { worstFitted } from '../combat/wear.js';
...
const gunMounts = () => ((S.stats && S.stats.mounts) || []).filter(w => w && w.kind !== 'utility');
...
export function canWarp() {
  ...
  // The arrays. Checked here rather than in `toggleWarp` so that everything which asks
  // "can this ship warp" — the key, the autopilot, the course planner, ARIA — gets the
  // same answer, with a reason. A constraint enforced at one call site is a constraint.
  const arrays = warpBlocked();
  if (arrays) return no('panels', arrays);
  return OK;
}
