...
export function describeMods(mods) {
  ...
  const LABEL = {
    ...
    // The thermal pair. Both have been summed by `fitBonuses` and read by `recalcStats`
    // since the heat model shipped; neither had a label, because until v1.02.60 no module
    // produced them and the fitting screen never had one to print.
    heatSinkAdd: ['Heat capacity', pct], heatVentMult: ['Radiators', pct]
  };
  const out = [];
  for (const k in mods) {
