...
import { transferRows } from '../flight/ephemeris.js';
import { lagrangePoints, anomalyAt, isWorked, pointDistance } from '../flight/lagrange.js';
...
import { holdCap, holdMass, manifestOf } from '../trade/holds.js';
...
/**
 * Where scanning is being done from, and with what.
 *
 * Docked, that is the station's own array rather than the ship's. A station is a fixed
 * installation with a dish the size of a hull, and `services.sensorRange` has been sitting
 * on every station since the service table was written with nothing reading it — the
 * `sensor` module even upgrades it by +2,200. It reads now. It is also what makes an
 * observation chart worth opening while you are on the pad: the view from the office deck
 * is *wider* than the view from the cockpit, not narrower.
 */
export function scanOrigin() {
  const st = S.docked;
  if (st) {
    const svc = (st.userData && st.userData.services) || {};
    return { pos: st.position, range: svc.sensorRange || 1800, from: st.userData.name };
  }
  return { pos: S.player.position, range: S.stats.sensor, from: 'ship' };
}

export function liveTier(obj) {
  if (!obj) return 0;
  const o = scanOrigin();
  const d = o.pos.distanceTo(obj.position);
  return tierAt(d, o.range, S.stats.scanTier || 0, attenuation(obj));
}
