...
import { canMine, announce } from '../platform/preflight.js';
import { witnessClaimJump } from '../npc/npc-brain.js';
import { AVATAR } from '../../core/config.js';
import { wearMining } from '../combat/wear.js';
import { debris } from '../../world/particles.js';
...
export function updateMining(dt) {
  ...
  // Chips coming off the cut, back along the beam.
  //
  // The count is the *yield*, not a constant: `got` is what this tick actually pulled out of
  // the rock, normalised against the best the fit could do, so a rich seam visibly throws more
  // material than a poor one. That makes "is this rock worth staying on" a question you can
  // answer by looking at it — the number was previously only available by watching the cargo
  // readout tick, which means looking away from the thing you are aiming at.
  const best = MINING.rate * S.stats.miningMult * dt;
  const richness = best > 0 ? Math.min(1, got / best) : 0;
  _back.copy(p.position).sub(rock.position).normalize();
  debris(rock.position, _back, 'ore', richness);
