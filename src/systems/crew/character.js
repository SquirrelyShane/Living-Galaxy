...
import { foundCompany, placeAtHQ } from '../company/company.js';
import { FRAMES, FRAME_KEYS, DEFAULT_FRAME, pronounsOf } from '../../data/origins.js';
import { SKILL_KEYS, LINEAGES, CORPORATIONS, CAREERS, agentFor, startingStanding,
         resolveCorp } from '../../data/origins.js';
import { playerDossier, adjustStanding, refreshRung, store } from '../company/dossier.js';
...
function emptyCharacter() {
  ...
  return {
    ...
    // Presentation and pronoun. Never a stat — see FRAMES in data/origins.js.
    frame: DEFAULT_FRAME,
 ...
 */
export function createCharacter({ name, lineage, corp, career, company, frame }) {
  // A save may carry an employer that has since been retired or renamed. Resolve first,
  // or a load path silently produces a null character. See CORP_ALIASES in data/origins.js.
  const resolved = resolveCorp(corp);
  const wasMigrated = resolved !== corp;
  corp = resolved;
  ...
  //
  // A *migrated* employer skips the check on purpose: the lineage's current option list is
  // about what creation offers today, and a character who really did work for an outfit
  // since folded into another does not stop having worked there. The flag is captured
  // before the reassignment above — comparing after it is always true, which is the bug
  // this comment exists to stop somebody reintroducing.
  if (!wasMigrated && !L.corps.includes(corp)) return null;

  const ch = emptyCharacter();
  ch.name = (name || '').trim().slice(0, 16) || 'Unnamed';
  ch.lineage = lineage;
  ch.frame = FRAME_KEYS.includes(frame) ? frame : DEFAULT_FRAME;
  ch.corp = corp;
  ch.career = career;
  ch.agent = K.agent;
  ch.created = true;
...
const levelCost = level => Math.round(CHAR.levelBase * Math.pow(CHAR.levelGrowth, level - 1));
...
export function characterSheet() {
  ...
  return {
    ...
    frame: ch.frame || DEFAULT_FRAME,
    frameName: (FRAMES[ch.frame] || FRAMES[DEFAULT_FRAME]).name,
    pronouns: pronounsOf(ch.frame),
    // Two different things that both read as "machine": the Forged lineage is one, and
    // presenting as a synthetic chassis is the other. A Forged pilot can present any way
    // they like, and a flesh pilot can wear a chassis.
    machine: !!(L && L.machine),
    machineRead: !!(L && L.machine) || !!(FRAMES[ch.frame] || {}).machineRead,
