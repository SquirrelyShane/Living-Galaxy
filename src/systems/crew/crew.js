...
import { held, takeMaterial } from '../industry/crafting.js';
import { sfx } from '../platform/audio.js';
import { registerEngineerCheck } from '../combat/wear.js';
import { canPilot } from '../company/career.js';
import { crewNote } from './crew-note.js';
...
export function initCrew() {
  ...
  // A career with no hull has nobody aboard it. The starting engineer and helm officer are
  // *ship's* crew — they exist because a pilot cannot run a reactor and fly at the same
  // time — and issuing them to a founder standing on an office deck produced a payroll for
  // two people serving on a ship that does not exist, plus provisions drawn for them. The
  // company hires captains for the hulls it signs; that is a different roster.
  if (!canPilot()) { clearCrew(); return; }
...
/**
 * Stand the roster down.
 *
 * Called when the character turns out to be a career that does not fly. `initCrew()` runs
 * at boot, *before* character creation on a new game — so an executive is issued a crew and
 * then has to be relieved of it. `createCharacter()` calls this on the way through.
 */
...
export function updateCrew(dt) {
  ...
  if (S.crewPayT >= CREW.wageInterval) {
    ...
    // The crew notice payroll either way. See systems/crew/crew-talk.js.
    crewNote(paid ? 'payday' : 'broke');
...
const clearPost = id => assignPost(id, specialtyOf(
