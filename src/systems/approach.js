...
import { openDock } from '../ui/dock.js';
...
export function startApproach() {
  ...
  status(`Approach — ${t.name}`);
  sfx.ui();
  return true;
}

export function cancelApproach(quiet) {
...
export function updateApproach(dt) {
  ...
  // Cap approach thrust at 25% so energy is not drained by continuous max burn
  p.throttle = clamp(want / S.stats.maxSpeed, -0.2, 0.25);
