...
import { net } from '../systems/platform/net.js';
import { startApproach, matchTarget } from '../systems/flight/approach.js';
import { inClaimedSpace } from '../systems/platform/worldsim.js';
import { scanPlanet, probePlanet, surveyLevel, planetInfo, asteroidDetail } from '../systems/industry/survey.js';
import { beginScan, scanReport, scanProgress, liveTier, TIER_NAME, scanOrigin } from '../systems/industry/scanner.js';
import { contacts as sensorContacts } from '../systems/flight/contacts.js';
import { lagrangeContacts, lagrangePoints, investigate, charted, isWorked, pointDistance } from '../systems/flight/lagrange.js';
import { heatFraction } from '../systems/combat/weapons.js';
import { fittedFeeds, magazineReport } from '../systems/combat/magazine.js';
import { HEAT, LAGRANGE } from '../core/config.js';
import { payroll } from '../systems/crew/crew.js';
import { setCourse, toggleWarp } from '../systems/flight/warp.js';
import { sfx } from '../systems/platform/audio.js';
import { openLockedDossier } from './dossier.js';
import { autopilotReport, toggleAutopilot } from '../systems/npc/autopilot.js';
import { playConn, abortConn } from './conn.js';
...
export function initHud() {
  ...
  if (E['ap-toggle']) E['ap-toggle'].addEventListener('click', () => {
    ...
    refreshAutopilot();
    sfx.ui();
    // The handoff sequence. Deliberately *after* the toggle and with its
    // return value ignored: ARIA is already flying by the time the first
    // frame of it draws, and if `ui/conn.js` were deleted the only thing
    // that would change is that you no longer see her take the ship.
    // Both directions get a sequence. The release is the shorter one — see CONN.releasePace.
    playConn(on ? 'bind' : 'release');
    // ...and the one question the needs model cannot answer for itself: what is this ship
    // for. Asked once, the first time she is ever given the ship, and available from the
    // chip under the switch after that. See ui/doctrine.js.
    if (on) doctrinePrompt(); else refreshDoctrineChip();
  });

  E['target-approach'].addEventListener('click', () => startApproach());
  E['target-match'].addEventListener('click', () => matchTarget());
  // Warp, from the panel that already knows what you are looking at.
  //
  // Locking a destination has laid the course since v1.02.52 (see `flight/targeting.js`),
  // so this does not need to set one — but it sets it again anyway, deliberately. The lock
  // may have been made before a ring or a Lagrange point moved, and re-laying it costs
  // nothing and removes the one case where the button flies you to a stale waypoint.
  if (E['target-warp']) E['target-warp'].addEventListener('click', () => {
    const t = S.target;
    if (!t) return;
    setCourse(t.obj, t.name);
    if (S.warp.state === 'idle') toggleWarp();
  });

  // The systems detail drawer. Remembered, because a pilot who closed it wants it closed
  // on the next flight too — and it is the kind of setting that is infuriating to re-set.
  if (E['systems-more']) {
    const drawer = () => {
      const open = !!S.settings.systemsDetail;
      E['systems-more'].classList.toggle('open', open);
      if (E['systems-extra']) E['systems-extra'].classList.toggle('open', open);
    };
    E['systems-more'].addEventListener('click', () => {
      S.settings.systemsDetail = !S.settings.systemsDetail;
      drawer();
      sfx.ui();
    });
    drawer();
...
/**
 * A hull the company owns is taking fire.
 *
 * Kept separate from `setThreat()` on purpose. That banner answers "am I in danger"; this
 * one answers "is something of mine being destroyed somewhere else", and a pilot who reads
 * the second as the first will break off a fight for no reason. Amber, slower blink, its
 * own line — and it escalates to critical when the hull is nearly gone, because at that
 * point the two questions have the same answer.
 */
export function setFleetAlert(name, hullFrac) {
  const on = !!name;
  setFlag('fleet-alert', 'show', on);
  if (!on) { setFlag('fleet-alert', 'critical', false); return; }
  const critical = hullFrac != null && hullFrac < 0.35;
  setText('fleet-alert', critical
    ? `⚠ ${name} — LOSING HER`
    : `⚠ ${name} UNDER FIRE`);
  setFlag('fleet-alert', 'critical', critical);
}
...
export function updateHud(dt, force = false) {
  ...
  if (force || slowT >= 0.1) {
    ...
    // watching this one specifically to know when they can move again, and "99%" for the
    // last second and a half of a retraction is the wrong answer to that question.
    const hab = habitatReport();
    setFlag('array-row', 'hidden', hab.arrays === 0);
    if (hab.arrays) {
      setWidth('array-fill', hab.pct);
      setText('array-val', hab.pctText);
      setFlag('array-fill', 'moving', hab.panels === 'deploying' || hab.panels === 'stowing');
    }
    // The farm, in the detail drawer with the rest of the once-a-minute numbers.
 ...
 *
 * Through the same diffing helpers as everything else, so a line that has not changed costs
 * nothing — and this one changes rarely and is read constantly, which is exactly the case
 * the write budget exists for.
 */
function refreshAutopilot() {
  const r = autopilotReport();
  setFlag('ap-toggle', 'on', r.on);
  setFlag('ap-toggle', 'off', !r.on);
  setText('ap-label', r.on ? 'AP ON' : 'AP OFF');
  setFlag('ap-line', 'hidden', !r.on);
  if (r.on) setText('ap-line', r.line || '—');
  // What she is for, under what she is doing. Only while she has the ship.
  setFlag('ap-doctrine', 'hidden', !r.on);
  if (r.on && r.doctrineName) setText('ap-doctrine', r.doctrineName);
}

function updateThrottleBar() {
  const t = S.player.throttle;
  setText('speed-value', Math.round(t * 100) + '%');
  const frac = clamp(((t + 0.25) / 1.25) * 100, 0, 100);
  setWidth('speed-fill', frac);
  setFlag('speed-fill', 'reverse', t < 0);
  // The thumb is where the relative drag is grabbed, so it has to be where the value is.
  // Through the same quantised diff as the bar: this is a per-frame write and the throttle
  // moves under the autopilot continuously.
  const thumb = E['speed-thumb'];
  if (thumb) {
    const q = Math.round(frac * 2) / 2;
    if (cache.get('speed-thumb\u0000L') !== q) {
      cache.set('speed-thumb\u0000L', q);
      thumb.style.left = q + '%';
      writes++;
    } else skipped++;
  }
...
function updateTargetPanel() {
  ...
  // teaches the player to ignore the panel. Same `NOT_A_PLACE` distinction the targeting
  // module draws, read through the kind rather than re-listing it here.
...
/**
 * The system's name in the top bar.
 *
 * Hardcoded as "SOLARIS SYSTEM" in index.html since the first build, which was true right
 * up until v1.02.33 made the system a seed output. Written once when the plan changes
 * rather than every frame — the name of the star you are orbiting is not volatile data.
 */
let namedSystem = null;
export function refreshSystemName() {
  const plan = S.systemPlan;
  const name = plan && plan.star && plan.star.name
    ? plan.star.name.replace(/ Prime$/, '') + ' System'
    : 'Unknown System';
  if (name === namedSystem) return;
  namedSystem = name;
  setText('system-name', name.toUpperCase());
}
...
function scanContacts() {
  ...
  // this list refused to carry them, so the field on the map had nothing to lock in the
...
function renderContacts() {
  ...
  setHtml('nearest-list', list.map((c, i) => {
    ...
    const own = c.mine ? ' mine' : c.own === 'pilot' ? ' pilot' : '';
