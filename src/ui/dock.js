...
import { serviceAll, serviceQuote } from '../systems/combat/wear.js';
import { openCrew } from './crew.js';
import { $, el, fmtCr, fmtMass } from '../core/utils.js';
import { priceFor, sell, sellAll, repair, repairQuote, upgradeCost, buyUpgrade, undock, buyProbe, probeCost, buyHull, ownsHull, hullPrice, upgradeLocked, upgradeReqText, buyWeapon, ownsWeapon, buyModule, ownsModule, sellModule } from '../systems/trade/economy.js';
import { saveGame } from '../systems/platform/save.js';
import { postPlayerJob, suggestedFee, openDeals, PLAYER } from '../systems/trade/deals.js';
...
import { characterSheet, spendPoint, hasLicence, licenceReq, licencePrice,
         licenceBlocker, buyLicence } from '../systems/crew/character.js';
import { currentMission } from '../systems/trade/missions.js';
import { boardFor, activeContracts, acceptContract, acceptBlocker, abandonContract,
         contractProgress, timeLeft, issuerOf, eligibility, sellableOf, deliverableAt,
         deliverConsignment } from '../systems/trade/contracts.js';
...
// Freight board state. Which commodity the board is quoting, and the bounds on one job:
// below FREIGHT_MIN there is nothing worth a hauler's time, and FREIGHT_MAX is one hold.
const FREIGHT_MIN = 100;
const FREIGHT_MAX = 2000;
let freightPick = 'ore';
...
function renderTrade() {
  ...
  if (hasCompany() && atHQ()) {
    ...
    body.appendChild(el('div', 'dock-note',
      `${co.name} headquarters · ${st.userData.name} · balance ${fmtCr(S.credits)}`));
    // Explicit entry point: on mobile the global ◈ Ops button is easy to miss behind the
    // dock overlay. Open the staff tab directly so the executive desk is one tap away.
    const cmd = el('button', 'buy-btn wide-btn', 'OPEN OPS — COMPANY COMMAND');
    cmd.style.margin = '8px 0 12px';
    cmd.addEventListener('click', () => { openOps('staff'); });
    body.appendChild(cmd);
  } else if (hasCompany()) {
    const co = company();
    body.appendChild(el('div', 'dock-note',
      `${co.name} · office at ${co.hqStation || 'unregistered'} · balance ${fmtCr(S.credits)}`));
    const cmd = el('button', 'buy-btn wide-btn', 'OPEN OPS — COMPANY COMMAND');
    cmd.style.margin = '8px 0 12px';
    cmd.addEventListener('click', () => { openOps('staff'); });
    body.appendChild(cmd);
  } else {
    // No charter yet. This is the only place the game says the executive layer exists at
    // all, so it says it here rather than leaving an empty Staff tab to be discovered.
    const reg = el('button', 'buy-btn wide-btn', 'REGISTER A COMPANY CHARTER');
    reg.style.margin = '8px 0 12px';
    reg.addEventListener('click', () => { openOps('staff'); });
    body.appendChild(reg);
  }
  body.appendChild(el('div', 'dock-note',
    `${st.userData.category} hub · hold ${fmtMass(cargoMass())} of ${fmtMass(S.stats.cargoCap)} · balance ${fmtCr(S.credits)}`));

  // Anything this station is waiting on comes first — it is why you flew here.
  const due = deliverableAt(st);
  if (due.length) {
    for (const c of due) {
      row(`Deliver — ${COMMODITIES[c.commodity].name}`,
        `${Math.round(c.loaded)} kg consigned · ${c.title}`,
        'Deliver', true, () => { deliverConsignment(st); render(); },
        fmtCr(c.pay));
    }
  }

  for (const key in COMMODITIES) {
    const held = Math.round(S.cargo[key]);
    const mine = Math.round(sellableOf(key));
    const locked = held - mine;
    const unit = priceFor(st, key);
    row(COMMODITIES[key].name,
      `${held} kg aboard · ${unit} cr/kg here` +
      (locked > 0 ? ` · ${locked} kg under consignment` : ''),
      'Sell', mine > 0, () => { sell(key); render(); },
      mine > 0 ? fmtCr(mine * unit) : '—');
  }
  const total = Object.keys(COMMODITIES).reduce((s, k) => s + Math.round(sellableOf(k)) * priceFor(st, k), 0);
  row('Sell everything', 'Empties the hold at local rates — consignments stay aboard',
      'Sell all', total > 0, () => { sellAll(); render(); }, fmtCr(total));
}
...
function renderFreight() {
  ...
  // v1.01.98: the board used to post `'ore'` and nothing else — hardcoded into both the fee
  // quote and the call. Everything underneath was already general: `postPlayerJob()` takes a
  // commodity, `dealValue()` prices it at the destination market, and `settle()` lands it
  // there. A pilot with a hold full of survey data could not post any of it, and the reason
  // was four characters in this file.
  const posted = Object.keys(COMMODITIES).filter(k => Math.floor(S.cargo[k] || 0) >= FREIGHT_MIN);
  ...
  if (!posted.length) {
    body.appendChild(el('div', 'dock-note',
      `Nothing in the hold worth moving — a job needs at least ${FREIGHT_MIN} kg of one ` +
      'commodity aboard. Ore, salvage and survey data all post.'));
    return;
  }
  if (!posted.includes(freightPick)) freightPick = posted[0];

  // One tap per commodity rather than a dropdown: three at most, and a phone should not have
  // to open a menu to change one word.
  if (posted.length > 1) {
    const strip = el('div', 'chiprow');
    for (const k of posted) {
      const b = el('button', 'chip' + (k === freightPick ? ' on' : ''),
        `${COMMODITIES[k].name} · ${Math.floor(S.cargo[k])} kg`);
      b.addEventListener('click', () => { freightPick = k; render(); });
      strip.appendChild(b);
    }
    body.appendChild(strip);
  }

  const com = freightPick;
  const kg = Math.min(Math.floor(S.cargo[com] || 0), FREIGHT_MAX);
  ...
  // station and not to another — which is the whole reason to show more than one. That is
  // more true now than it was: TRADE_MULT spreads survey data 0.80–1.50 across station
  // categories, nearly twice the spread ore has.
  for (const st of dests.slice(0, 4)) {
    const dest = st.userData.name;
    const fee = suggestedFee(com, kg, dest);
    row(`Ship ${kg} kg ${COMMODITIES[com].name.toLowerCase()} to ${dest}`,
      'A hauler takes it on their own terms — you pay on delivery',
      'Post', S.credits >= fee,
      () => {
        const deal = postPlayerJob({ commodity: com, kg, pay: fee, dest });
        if (deal) { S.cargo[com] = Math.max(0, (S.cargo[com] || 0) - kg); toast(`${deal.to} has your load`); }
      },
      fmtCr(fee));
  }
  ...
  for (const d of mine) {
    body.appendChild(el('div', 'dock-note',
      `In transit — ${d.kg} kg ${(COMMODITIES[d.commodity] || {}).name || d.commodity} ` +
      `to ${d.dest} with ${d.to} · ${fmtCr(d.pay)} on delivery`));
  }
}
...
function renderHulls() {
  ...
  for (const key of CLASS_ORDER) {
    ...
    // "In use" has to mean owned *and* flown. A founder starts licensed for the economic
    // hull with `classKey` pointing at it and no hull to their name, and the yard read that
    // as IN USE — telling a player who has just been refused an undock that they already
    // own the ship they were told to come here and buy.
    const active = owned && key === S.player.classKey;
...
<<<<<<< HEAD
  ...
  // The desk names itself, in its own colour and with its own doctrine under it. A board
  // that says "posting as independent" tells you nothing you can act on; a board that says
  // Halloway Assay, *unmeasured is unowned*, tells you what kind of work is on it and
  // whose opinion of you is about to move.
  const head = el('div', 'sec-head',
    `${st.userData.name} · <span style="color:${hex(power.color)}">${power.name}</span>`);
  body.appendChild(head);
  body.appendChild(el('div', 'empty-note',
...
<<<<<<< HEAD
    ...
    // short by how much, the standing short by how much, the certificate not held — so a
    // locked row reads as a target rather than as a refusal.
    const note = !gate.ok
      ? `<span class="urgent">Locked — ${gate.why}</span>`
      : `expires in ${clock(timeLeft(c))} · +${c.rep} standing · ${c.skill}`;
    row(
      c.title,
