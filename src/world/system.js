...
/**
 * Build the system described by the plan.
 *
 * v1.02.33: this used to walk two hardcoded arrays. It now walks `S.systemPlan`, which
 * `world/genesis.js` produces from the seed — so how many worlds there are, what class
 * each one is, where they sit, the star they go round and the fields between them are all
 * seed output rather than a table.
 *
 * The default is deliberate. With no plan set it falls back to the authored Solaris, which
 * is the system a save written before this patch is in and the one every existing suite
 * asserts against. Nothing has to opt *out* of proc-gen; a new game opts in, in `main.js`,
 * by planning a system before it builds one.
 */
export function createSystem(plan) {
  const p = plan || S.systemPlan || solarisPlan();
  S.systemPlan = p;
  resetLod();
  createStar(p.star);
  p.planets.forEach(createPlanet);
  p.stations.forEach(createStation);
}

function createStar(spec) {
  const st = spec || solarisPlan().star;
  const radius = st.radius || STAR.radius;
  const coronaR = st.corona || Math.round(radius * 2.44);
  const sun = new THREE.Mesh(
    new THREE.SphereGeometry(radius, 64, 64),
    new THREE.MeshBasicMaterial({ color: st.color || 0xfff0a0, fog: false })
  );
  // `gravity: 12` is the star's warp-well weight. It stays a game constant rather than
  // becoming a function of spectral class: every star is the largest well in its own
  // system, and scaling it down for an M-dwarf would make that system trivial to cross.
  sun.userData = { kind: 'star', name: st.name, radius, orbitRadius: 0, gravity: 12,
                   starClass: st.class || 'G', tempK: st.tempK || 5700 };
  // The corona shells used to be absolute radii tuned against a fixed 320-unit star.
  // Multiples instead, or a 200-unit red dwarf sits *inside* its own corona and an
  // 820-unit supergiant wears one three sizes too small.
  [[radius * 1.31, 0.22, 0xffcc55], [radius * 1.75, 0.10, 0xff8822], [coronaR, 0.05, 0xff5500]].forEach(([r, o, c]) => {
...
function createPlanet(pd) {
  ...
  g.userData = {
    ...
    orbitRadius: pd.orbit * ORBIT_SCALE,
    // Angular speed falls with the scaled radius, so a wider system is also a slower one —
    // which is the right way round: nothing should whip around a bigger orbit faster.
    orbitSpeed: ORBITAL_V.planet / (pd.orbit * ORBIT_SCALE), angle: wrand(0, TAU),
...
function createStation(st, i, all) {
  ...
  g.userData = {
    ...
    orbitRadius: st.orbit * ORBIT_SCALE,
    orbitSpeed: wrand(ORBITAL_V.station[0], ORBITAL_V.station[1]) / (st.orbit * ORBIT_SCALE),
    spinSpeed: wrand(0.05, 0.14),
    // Spread evenly around the star by index, so a system's berths are not all bunched on
    // one side of it. `all` is the plan's own station list — this used to divide by the
    // length of the hardcoded table, which was correct only for as long as that table was
    // the only thing that could be built.
    angle: (i / Math.max(1, (all && all.length) || 1)) * TAU + wrand(0, 0.5)
  };

  // core hull by category
  const metal = moduleMat;   // same cache — a station hull is metal in exactly the same way
  ...
  // Station floodlight, via the rig rather than a light of its own — twelve stations were
  // twelve permanent entries in every lit material's shader loop, for a glow you only see
  // from docking range.
  attachGlow(g, st.color, 0.7, 320);
  ...
  // one: at 96 units across a habitat ring genuinely covers several percent of the screen
...
export function addStation(spec) {
  ...
  // `spec.orbit` here is already a world radius — a caller placing a station by hand gives
  // the real distance, not a nominal one — so it is not scaled again.
...
// Module paint is keyed entirely by module type — every reactor on every station is the
// same green metal — but a fresh material was minted per fitted module, so a fully built
// system carried seventy materials that were pairwise identical. Cached by colour and
// finish instead; the geometry still varies with station size and stays per-module.
const moduleMats = new Map();
function moduleMat(c, m = 0.7, r = 0.35) {
  const k = c + '|' + m + '|' + r;
  let mat = moduleMats.get(k);
  if (!mat) {
    mat = new THREE.MeshStandardMaterial({ color: c, metalness: m, roughness: r,
      transparent: false, depthWrite: true });
    moduleMats.set(k, mat);
  }
  return mat;
}
