...
import { renderer, applyQuality as applyToRenderer } from './scene.js';
...
const starfieldScale = () => LEVELS[state.level].starfield;
...
export function setQualityLevel(n) {
  // The held point layer sizes itself against `effectScale()` and only repacks when something
  // marks it dirty. Change quality without this and the well shells keep the old budget until
  // the next unrelated edit — a lever that silently does not move, which is the exact failure
  // this project keeps finding. Imported lazily to keep quality free of world dependencies.
  import('./pointfield.js').then(m => m.invalidateFields()).catch(() => {});
