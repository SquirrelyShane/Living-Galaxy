 ...
 * right answer for a planet — a gas giant is legitimately visible across a system. See the
 * `range` note in `visibility.js` for why this is a separate reason from culling.
 */
export function register(obj, radius, levels, maxRange = 0) {
  if (!obj) return obj;
...
export function updateLod(viewportH) {
  ...
  for (let i = 0; i < registry.length; i++) {
    ...
    // Self-pruning, for the same reason the light rig prunes: NPCs are registered here
    // now, they despawn from half a dozen call sites, and a registry that only shrinks
    // when somebody remembers to call `unregister` is a leak with a long fuse. An object
    // that has left the scene is one whose parent is null — but only once it has been in
    // the scene at all, since a body is registered by its builder before the caller adds
    // it.
    if (obj.parent) l.seen = true;
    else if (l.seen) { registry.splice(i--, 1); obj.__lod = null; continue; }
    ...
    //
    // Through the arbiter rather than straight onto `.visible`: a docked hull is hidden
    // by the fleet layer for its own reason, and a bare assignment here would drag it
    // back into view the moment the player flew close enough.
