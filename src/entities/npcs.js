...
import { NPC_TYPES, NPC_RESPAWN_INTERVAL, AMBUSH, ENGAGE, POP, LOCK } from '../core/config.js';
...
function buildMesh(key, t) {
  ...
  if (key === 'drone') {
    g.add(new THREE.Mesh(new THREE.OctahedronGeometry(sz * 0.6), body(t.color)));
    g.add(new THREE.Mesh(new THREE.BoxGeometry(sz * 1.4, sz * 0.25, sz * 0.4),
      new THREE.MeshStandardMaterial({ color: 0x222233, metalness: 0.7, roughness: 0.4 })));
  } else if (key === 'command') {
    g.add(new THREE.Mesh(new THREE.IcosahedronGeometry(sz * 0.7, 1), body(t.color)));
    const ring = new THREE.Mesh(new THREE.TorusGeometry(sz * 1.1, 1.5, 8, 32),
      new THREE.MeshBasicMaterial({ color: 0xff66cc }));
    ring.rotation.x = Math.PI / 2;
    g.add(ring);
  } else {
    // nose points +Z so Object3D.lookAt orients it correctly
    const cone = new THREE.Mesh(new THREE.ConeGeometry(sz * 0.55, sz * 1.6, 5), body(t.color));
    cone.rotation.x = Math.PI / 2;
    g.add(cone);
    if (key === 'patrol') {
      const wing = new THREE.Mesh(new THREE.BoxGeometry(sz * 1.8, sz * 0.16, sz * 0.5),
        new THREE.MeshStandardMaterial({ color: 0x223344, metalness: 0.8, roughness: 0.3 }));
      wing.position.z = -sz * 0.2;
      g.add(wing);
    }
  }
  const glow = new THREE.PointLight(t.color, 0.5, 120);
  glow.position.z = sz * 0.8;
  g.add(glow);
  return g;
}
...
export function lockRange(u, see) {
  return (see !== undefined ? see : u.sensor) * LOCK.lockFactor;
}
