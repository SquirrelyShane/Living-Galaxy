...
**v1.01.96 "Out of Sight"** · save schema 17 · 3,261 checks across forty-one suites ·
`CHANGELOG.md` for the history, `docs/SLICE_PLAN.md` for how it was built,
`PATCH_v1.01.96.md` for what changed.
...
sockets, and owns everything durable — accounts, banked credits, per-system world deltas —
inside an encrypted vault. See **docs/SERVER.md** for the full guide.

```sh
./start.sh                       # everything: server up → browser open
                                 # (Windows: double-click launch.cmd)
node server/main.js              # or by hand — asks a vault passphrase, serves on :8765
```

TLS is automatic: the server issues its own certificate in pure Node at boot — no
openssl, no scripts — and reissues it when the laptop's address or name changes. Each
player accepts the browser warning once per device (or imports
`galaxy-data/certs/server.crt` as trusted).

**Players join by name, not by IP.** The server carries an mDNS beacon
(`server/beacon.js`): every device on the LAN can resolve **`galaxy.local`** with nothing
installed and no router settings — the standard every OS already uses to find printers.
Players open `https://galaxy.local:8765/` — the server serves the build — and put
`wss://galaxy.local:8765` in the boot screen (`GALAXY_NAME=nexis` renames it; the printed
IP is the fallback for the odd device that won't do mDNS). The server owns the world seed, so every
client generates the identical galaxy from it. A callsign with a passphrase is an
**account**: banked credits and a remembered position that survive a wiped browser or a
different device. Without one you still fly; you are a guest the galaxy forgets.

**The system is the shard.** The old relay fanned every packet to every connection. Now
your packets reach the pilots in the same *system* and nobody else: each occupied system is
a room, each room elects its own **host** (longest-connected; a sitting host is never
unseated by an arrival) who simulates the NPCs for that room, and jumping re-rooms you.
An empty system costs the server nothing, which is why millions of systems are affordable:
the durable data grows with what players *did*, never with what merely exists.

**No game logic on the server**, still, and still deliberate: it knows systems, identities,
money and differences-from-seed, not what a Nexis is. Stdlib-only Node — `npm install` is
not a step — so it runs anywhere Node 18+ does, including Termux.

**Remote pilots are drawn 280 ms in the past**, blended between two snapshots that have
*both already arrived*, with clocks synchronised against the server. Lose signal and your
slot is held (signed resume tickets, 15 minutes), so a phone that goes through a tunnel
comes back as itself.

**What is not shared:** pilot-versus-pilot damage resolution. Remote tracers are visual and
shots resolve on the shooter's client, though hits are now *addressed and room-checked* —
a hull two systems away cannot be hurt by a packet no matter what the packet says. The
server also runs a motion guard: kinematically impossible position reports are counted per
pilot and surfaced in `/api/status`.
