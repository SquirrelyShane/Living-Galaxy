...
}
{
  ...
  // haul credit comes from the sell hook, not from polling
  ...
  ok('a delivery to the wrong station does not count', haul.progress === 0);
  co.creditDelivery(S.world.stations[1], 'salvage', 80);
  ok('the wrong commodity does not count', haul.progress === 0);
  co.creditDelivery(S.world.stations[1], 'ore', 100);
  ok('the right delivery counts', haul.progress === 100);
  co.updateContracts(0.1);
