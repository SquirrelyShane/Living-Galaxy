...
}
{
  ...
  // v1.01.98: `deliverTo` returns the quantity that actually landed rather than a boolean,
  // because two things can shorten a delivery — the hold and the site's storage cap — and a
  // caller offering a quantity control needs to know which number to show afterwards.
  const steelBefore = CR.held('REF-001');
  CR.addMaterial('REF-001', 500);
  ok('materials can be sent down', IN.deliverTo(site.id, 'REF-001', 200) === 200);
  ok('they leave the hold', CR.held('REF-001') === steelBefore + 300);
  ok('sending more than held moves nothing', IN.deliverTo(site.id, 'REF-001', 9999) === 0);

  // The cap clamp itself. Production ticks have always respected storageCap(); a hand
  // delivery did not, so the ground store could be pushed past a ceiling the site's own
  // facilities honour.
  {
    const room = Math.floor(IN.storageCap(site) - IN.stored(site));
    CR.addMaterial('REF-001', room + 500);
    const landed = IN.deliverTo(site.id, 'REF-001', room + 400);
    ok('a delivery fills to the cap and no further', landed === room, `${landed} of ${room}`);
    ok('the store is at cap, not over it', IN.stored(site) <= IN.storageCap(site) + 0.001,
       `${IN.stored(site).toFixed(1)} / ${IN.storageCap(site)}`);
    ok('a full store takes nothing more', IN.deliverTo(site.id, 'REF-001', 50) === 0);
  }
}
{
