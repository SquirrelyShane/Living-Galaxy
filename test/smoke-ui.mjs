...
const hall = await page.evaluate(async () => {
  ...
  const out = { docked: sim.ship.dockedAt === st.id, port: st.name, sector: st.sector, cards: document.querySelectorAll("#sd-body .sd-person").length, radars: document.querySelectorAll("#sd-body .radar").length, lit: document.querySelectorAll("#sd-body .gcell.lit").length, crewJump: Boolean([...document.querySelectorAll("#sd-body .sd-sec h4")].find((h) => /^YOUR CREW/.test(h.textContent))) };
...
if (!hall.crewJump) fails.push("deck hall has no YOUR CREW section (0.3.49: crew live on the deck)");
