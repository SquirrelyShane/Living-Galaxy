...
let uid = 100;
// NOTE (v1.01.98): the blocks in this file share one `spawnNpc` id counter and one world
// RNG stream, so inserting a block that spawns ships changes which personas every block
// *below* it draws. Two assertions are sensitive to that — `somebody takes a fairly-priced
// job` and `a haul offer on the radio can become a real obligation` — because both depend on
// the traits of the particular characters that happen to spawn. Re-seeding inside `reset()`
// fixes the first and breaks the second, so it is not a one-line change; it is filed in
// bucket C of docs/OPEN_ITEMS.md. Until then: **add new blocks at the end of this file.**
const reset = () => {
...
// ── every commodity, not just ore ────────────────────────────────────
// The freight board hardcoded `'ore'` into both the fee quote and the call for eight
// slices (ui/dock.js), which made this look like an ore feature. It never was: nothing
// below the UI knows what ore is. Asserted per commodity so the next caller can rely on it.
console.log('\n— any commodity settles —');
{
  for (const com of Object.keys(COMMODITIES)) {
    reset();
    const miner = ship('miner'), hauler = ship('hauler', 400200);
    const dest = S.world.stations[0];
    const spec = { commodity: com, kg: 800, dest: dest.userData.name };
    spec.pay = Math.round(D.dealValue(spec) * 0.9);
    ok(`${com} is priced at the destination`, D.dealValue(spec) > 0, String(spec.pay));
    ok(`${com} quotes a fee`, D.suggestedFee(com, 800, dest.userData.name) > 0);
    const deal = D.propose(miner.userData, hauler.userData, spec);
    ok(`${com} finds a carrier`, !!deal);
    const before = bookFor(dest).stock[com];
    D.settle(deal);
    ok(`${com} lands on the destination market`, bookFor(dest).stock[com] > before,
       `${before} -> ${bookFor(dest).stock[com]}`);
  }
}
