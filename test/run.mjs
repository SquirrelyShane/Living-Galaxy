...
const { initProjectiles, updateProjectiles, fire, activeProjectiles } = await imp('systems/combat/projectiles.js');
const { initCombat, updateCombat, damagePlayer, damageNpc } = await imp('systems/combat/combat.js');
const { holdMass } = await imp('systems/trade/holds.js');
...
const { updateWarp, toggleWarp, setCourse } = await imp('systems/flight/warp.js');
const { startApproach, updateApproach, matchTarget, requestDocking, hailOpen, closeHail } = await imp('systems/flight/approach.js');
const { scanPlanet, probePlanet, asteroidDetail, surveyLevel } = await imp('systems/industry/survey.js');
const { updateTargeting, cycleTarget, setTarget, clearTarget } = await imp('systems/flight/targeting.js');
const { repairQuote, updateDocking, dock, undock, sell, sellAll, repair, buyUpgrade, upgradeCost, switchClass, buyHull, ownsHull, hullPrice, upgradeLocked, canUpgrade } = await imp('systems/trade/economy.js');
const { saveGame, loadGame, hasSave, wipeSave } = await imp('systems/platform/save.js');
const { initAudio } = await imp('systems/platform/audio.js');
const { initHud } = await imp('ui/hud.js');
const { initDock } = await imp('ui/dock.js');
const { initHail } = await imp('systems/flight/approach.js');
...
// v1.02.61: docking reach is measured from the *hull*, not the centre. The old assertion
// parked the ship a hundred units out and expected a pad — which on a thirty-unit berth is
// seventy kilometres of open space, and is exactly the "docking from miles away" this
// changed. Both sides of the line are asserted now, because a reach that only ever says
// yes is not a reach.
S.player.position.copy(station.position).x += berthR + DOCK.reach * 4;
updateDocking();
ok('no pad from well off the hull', S.dockCandidate === null,
