...
const { predict, intercept, transfer, separationAt, rateOf, orbitRadiusOf,
        transferRows, closingRate } = await imp('systems/flight/ephemeris.js');
const { foundSite, sites } = await imp('systems/industry/planetary.js');
...
const { fields, isRing, fieldMid, parentOf, fieldPoint, fieldDistance, fieldTarget,
        refreshFieldTarget, fieldContacts } = await imp('systems/flight/fields.js');
...
console.log('\n— Lagrange points —');
{
  ...
  // point arrives at now is whatever the pilot asked the jump for.
  ok('an arrival at a point is what the pilot asked for',
     arrivalRadius(t.userData) === WARP.closeArrive, String(arrivalRadius(t.userData)));
  ok('...and honours a standoff when one is set',
